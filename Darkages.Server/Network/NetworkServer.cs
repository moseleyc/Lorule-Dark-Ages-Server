...
namespace Darkages.Network
{
    ...
    {
        private MethodInfo[] handlers;
        private Socket listener;
        private bool listening;

        public IPAddress Address { get; private set; }
        public TClient[] Clients { get; private set; }

        public NetworkServer(int capacity)
        {
            var type = typeof(NetworkServer<TClient>);

            this.Address = ServerContext.IPADDRESS;
            this.Clients = new TClient[capacity];

            this.handlers = new MethodInfo[256];

            for (int i = 0; i < this.handlers.Length; i++)
            {
                this.handlers[i] = type.GetMethod(
        ...
        private void EndConnectClient(IAsyncResult result)
        {
            try
            {
                ...
                {
                    Socket = new NetworkSocket(this.listener.EndAccept(result))
                    {
                ...
                if (client.Socket.Connected)
                {
                    if (this.AddClient(client))
                    {
                        ...
                        client.Socket.BeginReceiveHeader(this.EndReceiveHeader, out error, client);
                ...
                if (this.listening)
                {
                    this.listener.BeginAccept(this.EndConnectClient, null);
                }
            }
            catch
            {
        ...
        private void EndReceiveHeader(IAsyncResult result)
        {
            ...
            {
                client.Socket.BeginReceivePacket(this.EndReceivePacket, out error, client);
            }
            else
            {
                client.Socket.BeginReceiveHeader(this.EndReceiveHeader, out error, client);
        ...
        private void EndReceivePacket(IAsyncResult result)
        {
            ...
            {
                this.ClientDataReceived(client, client.Socket.ToPacket());
        ...
        protected void HandleFormat(TClient client, NetworkFormat format)
        {
            ...
            if (this.handlers[format.Command] != null)
            {
                this.handlers[format.Command].Invoke(this, new object[]
                {
                    client,
                    format
                });
        ...
        public void RemoveClient(TClient client)
        {
            ...
            {
                if (this.Clients[i] != null &&
                    this.Clients[i].Serial == client.Serial)
                {
                    this.Clients[i] = null;
                    break;
                }
        ...
        public virtual void Start(int port)
        {
            ...
            this.listener.Listen(ServerContext.Config.ConnectionCapacity);
            this.listener.BeginAccept(this.EndConnectClient, null);
        }
        ...
        public void ProcessFormat(TClient client, NetworkPacket packet)
        {
            ...
            if (format != null)
            {
                ...
                if (this.handlers[format.Command] != null)
                {
                    this.handlers[format.Command].Invoke(this,
                        new object[]
                        {
                                client,
                                format
        ...
        private void RemoveAisling(TClient client)
        {
            ...
            foreach (var near in nearby)
            {
                ...
                if (near.LoggedIn)
                {
                    ...
                    {
                        near.Map.Tile[(client as GameClient).Aisling.X, (client as GameClient).Aisling.Y] = Types.TileContent.None;
                    }
                    near.Show(Types.Scope.Self, new ServerFormat0E((client as GameClient).Aisling.Serial));
                }
            }
        }
    }
